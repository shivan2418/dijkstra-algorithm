...
var Node = /** @class */ (function () {
    function Node(x, y, cost) {
        ...
        this.end = false;
        this.visited = false;
        this.distance_from_start = Infinity;
    }
    Node.prototype.toString = function () {
        return "(".concat(this.x, ",").concat(this.y, ")");
    };
    Node.prototype.get_distance_to_previous = function (previous) {
        var cost = previous.cost + this.cost;
        return cost;
    };
    Node.prototype.get_distance_from_start = function (arrived_via) {
        var cost = this.cost;
        var previous = this.previous_node || arrived_via;
    ...
    Node.prototype.get_unvisited_neighbors = function () {
        var neighbors = this.get_neighbors();
        return neighbors.filter(function (neighbor) { return !neighbor.visited; });
    };
    Node.get_optimal_path = function (nodes) {
        var end_node = _.find(nodes, { end: true });
        var path = [];
    ...
    Node.get_next_node_to_visit = function (nodes) {
        nodes = nodes.filter(function (node) { return !node.visited; });
        nodes = nodes.sort(function (a, b) { return a.distance_from_start - b.distance_from_start; });
        return _.first(nodes);
    };
    Node.prototype.get_up = function (nodes) {
        return _find_node(this.x, this.y - 1, nodes);
    };
    Node.prototype.get_down = function (nodes) {
        return _find_node(this.x, this.y + 1, nodes);
    };
    Node.prototype.get_left = function (nodes) {
        return _find_node(this.x - 1, this.y, nodes);
    };
    Node.prototype.get_right = function (nodes) {
        return _find_node(this.x + 1, this.y, nodes);
    };
    return Node;
}());
exports["default"] = Node;
function _find_node(x, y, nodes) {
    return nodes.find(function (node) { return node.x === x && node.y === y; });
}
...
var sample_input = "1163751742\n" +
    ...
    "1293138521\n" +
    "2311944581";
var input = "7135912411912798932871391322889941544645211112288183969191588665579426181549613954113914616349281119\n" +
...
function parse_input(input) {
    ...
    rows.forEach(function (row, index) {
        Array.from(row).forEach(function (char) {
            var cost = parseInt(char);
            var node = new node_1["default"](x, y, cost);
            nodes.push(node);
            x++;
        });
        y++;
...
function solve(input) {
    var nodes = parse_input(input);
    // start at the start node
    var current_node = nodes.find(function (node) { return node.start; });
    while (!(current_node === null || current_node === void 0 ? void 0 : current_node.end)) {
        var nodes_to_check = current_node.get_unvisited_neighbors();
        nodes_to_check.forEach(function (node) {
            var dist_from_start = node.get_distance_from_start(current_node);
        ...
        current_node = node_1["default"].get_next_node_to_visit(nodes);
        if (current_node.end) {
            break;
        }
    }
    var end_node = nodes.find(function (node) { return node.end; });
    console.log(end_node.get_distance_from_start());
}
solve(sample_input);
solve(input);

},{"./classes/node":1,"lodash":3}],3:[function(require,module,exports){
